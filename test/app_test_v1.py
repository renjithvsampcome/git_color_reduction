...
def binary_search_color_reduction(frames: List[Image.Image], 
                                ...
                                max_colors: int,
                                duration: int,
                                temp_dir: str) -> Tuple[bytes, float, int]:
    """
    Enhanced binary search for optimal GIF compression using multiple techniques
    """
    best_result = None
    best_size = float('inf')
    best_colors = None
    ...
    optimization_strategies = [
        {'optimize': True, 'disposal': 2},  # Optimize with dispose previous frame
        {'optimize': True, 'disposal': 1},  # Optimize with keep previous frame
        {'optimize': True, 'transparency': 0},  # Try with transparency
    ...
    while max_colors - min_colors > 2:
        ...
        for strategy in optimization_strategies:
            ...
            try:
                ...
                reduced_frames = []
                for frame in frames:
                    # Try different dithering methods if needed
                ...
                # Apply the current optimization strategy
                ...
                if current_size <= target_size:
                    ...
                    if abs(current_size - target_size) < target_size * 0.1:
                        return best_result, best_size, best_colors
                
                if current_size > target_size:
                    max_colors = current_colors
                    break  # Try fewer colors
                else:
    ...
    # If we couldn't reach target size, try one final optimization with minimum colors
...
def optimize_gif(input_bytes: bytes, target_size_kb: float) -> Tuple[bytes, float, float, int]:
    """
    Optimized GIF compression using color reduction
    """
    ...
    try:
        ...
        # Analyze image and estimate parameters
        ...
        # Try color reduction
        try:
            optimized_data, achieved_size, final_colors = binary_search_color_reduction(
                frames,
                target_size_kb,
                min_colors=8,
            ...
            return optimized_data, achieved_size, compression_ratio, final_colors
...
def main():
    ...
    st.write("""
    This GIF compressor reducing file size through intelligent color reduction. 
        ...
        try:
            ...
            if st.button("Reduce Colors"):
                with st.spinner("Analyzing and reducing colors..."):
                    try:
                        optimized_data, achieved_size, compression_ratio, colors = optimize_gif(
